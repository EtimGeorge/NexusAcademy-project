...
const popularCourses = [
    { title: "AI for Content & Marketing", link: "/#/signup", imageUrl: "https://images.pexels.com/photos/879109/pexels-photo-879109.jpeg?auto=compress&cs=tinysrgb&w=120&h=120&dpr=1" },
    { title: "The Agentic Architect Program", link: "/#/signup", imageUrl: "https://images.pexels.com/photos/546819/pexels-photo-546819.jpeg?auto=compress&cs=tinysrgb&w=120&h=120&dpr=1" }
];
...
export async function render() {
    ...
    const pageHtml = await (await fetch('/src/pages/BlogPage/BlogPage.html')).text();
    ...
    popularCoursesList.innerHTML = popularCourses.map(createPopularCourseHtml).join('');

    return element;
}

export function init() {
    if (typeof anime === 'undefined') return console.error('Anime.js not loaded!');
...
function renderFilteredPosts() {
    ...
    if (currentFilters.searchTerm) {
        ...
        filteredPosts = filteredPosts.filter(post => post.title.toLowerCase().includes(searchTermLower));
    }
...
function renderAllPosts(postsGrid, featuredContainer) {
    ...
    postsGrid.innerHTML = standardPosts.map(post => createBlogPostCardHtml(post)).join('');
...
function initHeaderAnimation() {
    anime({ targets: '.animate-header', translateY: [20, 0], opacity: [0, 1], delay: anime.stagger(150), duration: 800, easing: 'easeOutExpo' });
}
function initScrollAnimations() {
    ...
    animatedElements.forEach((el, index) => { el.dataset.delay = index % 5; observer.observe(el); });
}